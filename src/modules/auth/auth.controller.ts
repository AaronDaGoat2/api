...
import { enviarNotificacionLogin } from '../mail/mail.service';
...
export class AuthController {
  ...
  static async login(req: Request, res: Response): Promise<void> {
    try {
      ...
      // 2. MASHUP: se ejecutan en paralelo la autenticación (fuente propia)
      // y la notificación por correo (fuente externa - Resend API).
      // Si el proveedor externo falla, no afecta el resultado del login.
      const [result, notificacion] = await Promise.all([
        AuthService.login(validation.data),
        enviarNotificacionLogin({ destinatario: validation.data.email }),
      ]);
      ...
      res.status(200).json({
        ...
        data: {
          ...
          mashup: {
          notificacionEmail: notificacion.enviado,
        },
      });
    } catch (error: any) {
      res.status(401).json({
